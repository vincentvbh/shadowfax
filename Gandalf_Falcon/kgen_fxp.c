...
/* see kgen_inner.h */
void
vect_mul_selfadj_fft(unsigned logn, fxr *restrict a, const fxr *restrict b)
{
	size_t hn = (size_t)1 << (logn - 1);
	for (size_t i = 0; i < hn; i ++) {
		a[i] = fxr_mul(a[i], b[i]);
		a[i + hn] = fxr_mul(a[i + hn], b[i]);
	}
}
...
/* see kgen_inner.h */
void
vect_div_selfadj_fft(unsigned logn, fxr *restrict a, const fxr *restrict b)
{
	size_t hn = (size_t)1 << (logn - 1);
	for (size_t i = 0; i < hn; i ++) {
		a[i] = fxr_div(a[i], b[i]);
		a[i + hn] = fxr_div(a[i + hn], b[i]);
	}
}
